...
def get_validation_data(env,controller,steps=1_000,device="cpu"):
    ...
    for i in range(0,steps,env.n):
        u = controller(x)
        x_data[i:i+n] = x.view(-1,env.x_dim)
        u_data[i:i+n] = u.view(-1,env.u_dim)
        x,_ = env.step(u)
    return torch.utils.data.TensorDataset(x_data[:i+n],u_data[:i+n]) #cut off trailing zeros, make dataset
